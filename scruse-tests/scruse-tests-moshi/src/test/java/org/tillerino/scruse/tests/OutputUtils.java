...
public class OutputUtils {
    ...
    public static String withJsonWriter(FailableConsumer<JsonWriter, IOException> output) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        Sink sink = Okio.sink(baos);
        BufferedSink bufferedSink = Okio.buffer(sink);
        JsonWriter generator = JsonWriter.of(bufferedSink);
        // !!! Enable this to match Jackson's behavior
        generator.setSerializeNulls(true);
    ...
    public static <T> String serialize(T obj, FailableBiConsumer<T, JsonWriter, IOException> output)
            throws IOException {
        return withJsonWriter(generator -> output.accept(obj, generator));
    }

    public static <T, U> String serialize2(T obj, U obj2, FailableTriConsumer<T, JsonWriter, U, IOException> output)
            throws IOException {
        return withJsonWriter(generator -> output.accept(obj, generator, obj2));
    }
    ...
    public static <T> String assertIsEqualToDatabind(
        ...
        String databind = new ObjectMapper().writeValueAsString(obj);
        String ours = withJsonWriter(generator -> output.accept(obj, generator, new SerializationContext()));
        assertThatJson(ours).isEqualTo(databind);
        return ours;
    }
