...
class ScalarListsReaderTest {
	...
	void testBooleanList() throws IOException {
		for (String json : JsonData.BOXED_BOOLEAN_ARRAYS) {
			List<Boolean> list = InputUtils.assertIsEqualToDatabind(json, impl::readBooleanList, new TypeReference<>() {
			});
			if (list != null) {
	...
	void testIntegerList() throws IOException {
		for (String json : JsonData.BOXED_INT_ARRAYS) {
			List<Integer> list = InputUtils.assertIsEqualToDatabind(json, impl::readIntegerList, new TypeReference<>() {
			});
			if (list != null) {
				assertThat(list).isInstanceOf(ArrayList.class);
			}
	...
	void testDoubleList() throws IOException {
		for (String json : JsonData.BOXED_DOUBLE_ARRAYS) {
			List<Double> list = InputUtils.assertIsEqualToDatabind(json, impl::readDoubleList, new TypeReference<>() {
			});
			if (list != null) {
				assertThat(list).isInstanceOf(ArrayList.class);
			}
	...
	void testCharList() throws IOException {
		...
		for (String json : jsons) {
			List<Character> list = InputUtils.assertIsEqualToDatabind(json, impl::readCharacterList, new TypeReference<>() {
			});
			if (list != null) {
				assertThat(list).isInstanceOf(ArrayList.class);
			}
	...
	void testStringList() throws IOException {
		for (String json : JsonData.STRING_ARRAYS) {
			List<String> list = InputUtils.assertIsEqualToDatabind(json, impl::readStringList, new TypeReference<>() {
			});
			if (list != null) {
				assertThat(list).isInstanceOf(ArrayList.class);
			}
