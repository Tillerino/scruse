...
class ScalarArraysReaderTest {
    ...
    void testCharArray() throws IOException {
        for (String json : JsonData.STRINGS) {
            assertIsEqualToDatabind(json, impl::readCharArray, new TypeReference<>() {});
        }
    }
    ...
    /**
     * Whereas, char[] is serialized as a String, Jackson databind does not support Character[]. This test is here so
     * that the overall method count in this class is as expected.
     */
    @Test
    ...
    void testEnumArray() throws IOException {
        for (String json : JsonData.ENUM_ARRAYS) {
            assertIsEqualToDatabind(json, impl::readEnumArray, new TypeReference<>() {});
        }
    }
    ...
    void testLargeIntArray() throws IOException {
		...
		""";
        assertIsEqualToDatabind(json, impl::readIntArray, new TypeReference<int[]>() {});
    }
}
