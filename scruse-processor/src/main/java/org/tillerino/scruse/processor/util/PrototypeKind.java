...
public sealed interface PrototypeKind {
	...
	record Input(TypeMirror jsonType, TypeMirror javaType, List<InstantiatedVariable> otherParameters) implements PrototypeKind { }

	record Output(TypeMirror jsonType, TypeMirror javaType, List<InstantiatedVariable> otherParameters) implements PrototypeKind { }

	record ReturningOutput(TypeMirror jsonType, TypeMirror javaType, List<InstantiatedVariable> otherParameters) implements PrototypeKind { }
	...
	static Optional<PrototypeKind> of(InstantiatedMethod m) {
		if (m.element().getAnnotation(JsonInput.class) != null
				...
				&& List.of(JACKSON_JSON_PARSER, JACKSON_JSON_NODE, GSON_JSON_READER, FASTJSON_2_JSONREADER).contains(m.parameters().get(0).type().toString())) {
			return Optional.of(new Input(m.parameters().get(0).type(), m.returnType(), m.parameters().subList(1, m.parameters().size())));
		}
		if (m.element().getAnnotation(JsonOutput.class) != null
				&& m.returnType().getKind() == TypeKind.VOID
				&& m.parameters().size() >= 2
				&& List.of(JACKSON_JSON_GENERATOR, GSON_JSON_WRITER, FASTJSON_2_JSONWRITER).contains(m.parameters().get(1).type().toString())) {
			return Optional.of(new Output(m.parameters().get(1).type(), m.parameters().get(0).type(), m.parameters().subList(2, m.parameters().size())));
		}
		if (m.element().getAnnotation(JsonOutput.class) != null
				&& m.returnType().getKind() != TypeKind.VOID
				&& !m.parameters().isEmpty()
				&& List.of(JACKSON_JSON_NODE).contains(m.returnType().toString())) {
			return Optional.of(new ReturningOutput(m.returnType(), m.parameters().get(0).type(), m.parameters().subList(1, m.parameters().size())));
		}
