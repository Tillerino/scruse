...
import org.tillerino.scruse.helpers.MoshiJsonReaderHelper;
...
public class MoshiJsonReaderGenerator extends AbstractReaderGenerator<MoshiJsonReaderGenerator> {
    private final VariableElement parserVariable;

    public MoshiJsonReaderGenerator(
    ...
    public MoshiJsonReaderGenerator(
            ...
            LHS lhs,
            MoshiJsonReaderGenerator parent,
    ...
    protected Snippet objectCaseCondition() {
        return Snippet.of("$T.isBeginObject($L)", MoshiJsonReaderHelper.class, parserVariable.getSimpleName());
    }
    ...
    protected Snippet nullCaseCondition() {
        return Snippet.of("$T.isNull($L)", MoshiJsonReaderHelper.class, parserVariable.getSimpleName());
    }

    private TypeElement jsonToken() {
        return utils.elements.getTypeElement("com.squareup.moshi.JsonReader.Token");
    }
