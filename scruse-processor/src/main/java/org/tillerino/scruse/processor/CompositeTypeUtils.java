...
public record CompositeTypeUtils(AnnotationProcessorUtils utils) {
        ...
        if (element.getKind() == ElementKind.RECORD) {
            ...
            for (ExecutableElement method : ElementFilter.methodsIn(element.getEnclosedElements())) {
                ...
                if (modifiers.contains(Modifier.STATIC)
                        ...
                        || utils.isJsonIgnore(annotationMirrors)
                ...
                if (name.startsWith("is")
                        ...
                        && (method.getReturnType().getKind() == TypeKind.BOOLEAN
                                || utils.types.isSameType(method.getReturnType(), utils.commonTypes.boxedBoolean))) {
    ...
    sealed interface CompositeTypeComponent {
        ...
        record RecordComponent(RecordComponentElement element) implements CompositeTypeComponent {
            ...
            @Override
            public void getToVariable(
                    String compositeVariableName, String targetVariableName, MethodSpec.Builder builder) {
                builder.addStatement(
                        "$T $L = $L.$L()", typeMirror(), targetVariableName, compositeVariableName, propertyName());
            }
        }
